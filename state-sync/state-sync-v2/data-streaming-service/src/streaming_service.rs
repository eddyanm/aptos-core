...
impl<T: AptosDataClient + Send + Clone + 'static> DataStreamingService<T> {
    ...
    fn update_progress_of_data_stream(
        ...
        // If the client dropped the listener, terminate the stream
        let data_stream = self.get_data_stream(data_stream_id);
        if data_stream.client_dropped_listener() {
            info!(
            ...
            metrics::CLIENT_DROPPED_LISTENER.inc();
            if self.data_streams.remove(data_stream_id).is_none() {
                return Err(Error::UnexpectedErrorEncountered(format!(
                    "Failed to terminate stream id: {:?}! It was not found in the streams!",
                    data_stream_id
                )));
            }
            return Ok(());
